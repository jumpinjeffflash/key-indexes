...
## Big Mac Index
    bigmac = quandl.get("ECONOMIST/BIGMAC_USA")
    bigmaclocal = bigmac[["local_price"]]
...
### VISUAL: BIG MAC INDEX    
    ...
    bigmaclocal_edit = bigmaclocal.rename(columns = {"Value":"Big Mac price ($)"})

    st.line_chart(data = bigmaclocal_edit, width=0, height=0, use_container_width=True)
